...
Item { // Wrapper
    ...
    property var searchActions: [
        {
            action: "img",
            execute: () => {
                executor.executeCommand(`${Directories.wallpaperSwitchScriptPath} --select`);
            }
        },
        {
            action: "dark",
            execute: () => {
                executor.executeCommand(`${Directories.wallpaperSwitchScriptPath} --mode dark`);
            }
        },
        {
            action: "light",
            execute: () => {
                executor.executeCommand(`${Directories.wallpaperSwitchScriptPath} --mode light`);
            }
        },
        {
            action: "accentcolor",
            execute: args => {
                executor.executeCommand(`${Directories.wallpaperSwitchScriptPath} --color ${args != '' ? ("'" + args + "'") : ""}`);
            }
        },
        {
            action: "todo",
    ...
    Process {
        ...
        function executeCommand(command) {
            executor.command = baseCommand.concat(`${command}`);
        ...
        ColumnLayout {
            ...
            ListView { // App results
                ...
                delegate: SearchItem {
                    ...
                    entry: modelData
                    query: root.searchingText.startsWith(ConfigOptions.search.prefix.clipboard) ? root.searchingText.slice(ConfigOptions.search.prefix.clipboard.length) : root.searchingText
                }
            }
        }
    }
